...
if ($data['result'] == 'success' && HAS_SEND_EMAIL == true) {
  ...
  $search = ['%subject%', '%name%', '%email%', '%message%', '%service%', '%castom%', '%date%'];
  $replace = [EMAIL_SETTINGS['subject'], $data['form']['name'], $data['form']['email'], $data['form']['message'], $data['form']['service'], $data['form']['castom'], date('d.m.Y H:i')];
...
if ($data['result'] == 'success' && HAS_WRITE_TXT) {
  ...
  $output .= 'Сообщение: ' . $data['form']['service'] . PHP_EOL;
  $output .= 'Сообщение: ' . $data['form']['castom'] . PHP_EOL;
  if (count($attachs)) {
